...
func (v Value) BoolValue(ec *EvalContext) (bool, error) {
	switch v.Type {
	case TypeBool:
...
func (v Value) DecimalValue(ec *EvalContext) (decimal.Decimal, error) {
	switch v.Type {
	case TypeBool:
		if v.boolValue {
...
func (v Value) StringValue(ec *EvalContext) (string, error) {
	switch v.Type {
	case TypeBool:
		if v.boolValue {
			return "TRUE", nil
...
func (v Value) Link() (*Link, error) {
	...
	case TypeBool, TypeDecimal, TypeString:
		return nil, NewError(ErrorKindCasting, "unable to use static value as a link")
	case TypeCell:
		return v.link, nil
	//case TypeRange:
